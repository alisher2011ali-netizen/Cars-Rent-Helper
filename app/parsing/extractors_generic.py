...
def determine_extractor_auto(pdf_text: str) -> type:
    ...
    if len(supported_extractors) > 1:
        raise exceptions.InputFileStructureError(
            f"Непонятный формат выписки. Больше чем один экстрактор говорят, "
...
def determine_extractor_by_name(extractor_name: str) -> type:
    ...
    raise exceptions.UserInputError(
        f'Указанный формат файла "{extractor_name}" Неизвестен.\n См. список известных форматов \n {extractor_names_set}'
    )
...
-------------------------------------------
    ...
    print(
        f"1) Extractor {extractor_type_object.__name__} initilyzed with the text from the file '{test_text_file_name}'\n\n"
    )
    print(
        f"1) Extractor {extractor_type_object.__name__} initilyzed with the knownly wrong text {wrong_text}"
    )
    ...
    try:
        ...
        raise exceptions.TestingError(
            "function 'check_specific_signatures()' shall raise exception 'exceptions.InputFileStructureError'"
        )
    ...
    try:
        ...
        raise exceptions.TestingError(
            "function 'get_period_balance()' shall raise exception 'exceptions.InputFileStructureError'"
        )
    except exceptions.InputFileStructureError:
        pass
    ...
    try:
        ...
        raise exceptions.TestingError(
            "function 'split_text_on_entries()' shall raise exception 'exceptions.InputFileStructureError'"
        )
    except exceptions.InputFileStructureError:
        pass
    ...
    for entry in entries:
        ...
        all_actually_returned_fields_set = all_actually_returned_fields_set | set(
            entry.keys()
        )
    ...
    try:
        ...
        raise exceptions.TestingError(
            "function 'get_entries()' shall raise exception 'exceptions.InputFileStructureError'"
        )
    except exceptions.InputFileStructureError:
        pass
    ...
    undefined_fiels_set = all_actually_returned_fields_set - set(
        columns_info_dic.keys()
    )
...
Specifically the following fields are missing {undefined_fiels_set}""")
    ...
    column_name_for_balance_calculation = (
        extractor.get_column_name_for_balance_calculation()
    )
    print(
        f"get_column_name_for_balance_calculation = {column_name_for_balance_calculation}"
    )
...
""")
    ...
    defined_but_not_used_fiels_set = (
        set(columns_info_dic.keys()) - all_actually_returned_fields_set
    )
