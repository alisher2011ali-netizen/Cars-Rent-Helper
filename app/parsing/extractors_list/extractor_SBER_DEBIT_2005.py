...
class SBER_DEBIT_2005(Extractor):
    ...
    def check_specific_signatures(self):
        ...
        test2 = re.search(
            r"Выписка по счёту дебетовой карты", self.bank_text, re.IGNORECASE
        )
        # print(f"{test2=}")
    ...
    def get_period_balance(self) -> Decimal:
        ...
        if res := re.search(
            r"СУММА ПОПОЛНЕНИЙ\t(\d[\d\s]*\,\d\d)", self.bank_text, re.MULTILINE
        ):
        ...
        if res := re.search(
            r"СУММА СПИСАНИЙ\t(\d[\d\s]*\,\d\d)", self.bank_text, re.MULTILINE
        ):
    ...
    def decompose_entry_to_dict(self, entry: str) -> dict:
        ...
        result["value_account_currency"] = get_decimal_from_money(line_parts[2], True)
        result["remainder_account_currency"] = get_decimal_from_money(line_parts[3])
        ...
        if len(line_parts) < 2 or len(line_parts) > 3:
            raise exceptions.Bank2ExcelError("Line is expected to 2 or parts :" + line)
        ...
        if len(line_parts) == 3:
            found = re.search(
                r"[(](.*?)(\w\w\w)[)]", line_parts[2]
            )  # processing string like (33,31 EUR)
            if found:
                ...
                raise exceptions.InputFileStructureError(
                    "Ошибка в обработке текста. Ожидалась структура типа (33,31 EUR), получено: "
                    + line
                )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        extractors_generic.debug_extractor(
            SBER_DEBIT_2005, test_text_file_name=sys.argv[1]
        )
