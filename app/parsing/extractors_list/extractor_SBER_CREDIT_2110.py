...
class SBER_CREDIT_2110(Extractor):
    ...
    def check_specific_signatures(self):
        ...
        test_vipiska_po_schetu_kereditnoy = re.search(
            r"Выписка по счёту кредитной карты", self.bank_text, re.IGNORECASE
        )
        # print(f"{test2=}")

        test_data_formirovaniya = re.search(
            r"Дата формирования документа", self.bank_text, re.IGNORECASE
        )
    ...
    def decompose_entry_to_dict(self, entry: str) -> dict:
        ...
        result["operation_date"] = datetime.strptime(
            result["operation_date"], "%d.%m.%Y %H:%M"
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        extractors_generic.debug_extractor(
            SBER_CREDIT_2110, test_text_file_name=sys.argv[1]
        )
