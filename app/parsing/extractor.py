...
class Extractor(ABC):
    ...
    def check_support(self) -> bool:
        ...
        try:
            ...
            result = (
                isinstance(self.get_period_balance(), Decimal)
                and len(self.split_text_on_entries()) > 0
            )
        ...
        for entry in self.split_text_on_entries():
            try:
                ...
                if isinstance(result, dict):
                    ...
                    raise RuntimeError(
                        "decompose_entry_to_dict() shall return either dict or list[dict]"
                    )

            except Exception as e:
                raise RuntimeError(
                    "Ошибка при обработке трансакции\n"
                    + "-" * 20
                    + "\n"
                    + entry
                    + "\n"
